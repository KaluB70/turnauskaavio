...
export class WeeklyTournamentService {
  ...
  private createAlternatingGroupMatches(group1Players: WeeklyPlayer[], group2Players: WeeklyPlayer[]): void {
    ...
    // Now alternate between groups when adding matches
    const maxMatches = Math.max(group1Matches.length, group2Matches.length);
    let group1Index = 0;
    let group2Index = 0;

    for (let i = 0; i < maxMatches * 2; i++) {
      if (i % 2 === 0) {
        // Add group 1 match
        if (group1Index < group1Matches.length) {
        ...
        // Add group 2 match
        if (group2Index < group2Matches.length) {
  ...
  findNextMatch(): void {
    ...
    if (this.currentPhase === 'group') {
      ...
      // All group matches complete, check if we need playoffs
      this.currentMatch = null;
      ...
      // Playoffs complete, move to final
      this.currentMatch = null;
  ...
  private checkPhaseTransition(): void {
    ...
    if (this.tournamentType === 'round-robin') {
      if (this.players.length === 3) {
        // With 3 players, treat group phase as final
        this.currentPhase = 'final';
        this.completeTournament();
        return;
      } else {
  ...
  completeMatch(winnerId: number): void {
    ...
    if (isFinalMatch) {
      // For final matches, go directly to completion without animation
      this.completeTournament();
      return;
    } else {
  ...
  getRoundForMatch(match: WeeklyMatch): string {
    if (match.round === 'group') {
      if (this.tournamentType === 'group-based') {
